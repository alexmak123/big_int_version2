/*  ЧТО ВАЖНО И НА ЧТО ОБРАЩАТЬ ВНИМАНИЕ
    1)выделение и утечка памяти
    2)правильные указатели
    3)наличие assertов для каждого блока (максимальная декомпозиция)
    4)сделать функцию normalize, которая будет приводить все числа к единому стандарту, например
    убирает лидируещие нули, преобразует -0 -> 0 и будем вызывать ее в самом конце непосредственно перед выводом числа
    ()
    5)сделать функцию check, которая проверяет все ли цифры меньше основания системы счисления и >= 0 и т.д. (...)
    6)в main все тесты, все остальное в bn_Makarchuk.c
    7)написанные функции в bn_Makarchuk.c ничего не должны вводить и выводить!
    8)все ошибочные ситуации должны быть обработаны без вывода чего-либо на экран
    9)функции, возвращающие указатели, должны при ошибке возвращать NULL, остальные функции должны возвращать 0 при успешном
    завершении операции и код ошибки из перечислимого типа bn_codes при неуспешном
    10) для деления у нас такая таблица:
        17 / 10 = 1
        17 % 10 = 7
        -17 / 10 = -2
        -17 % 10 = 3
        ...
        -17 / -10 = 1
        -17 % -10 = -7
    11)чекать указатели на работоспособность bn *r = (bn *)malloc (sizeof (bn));
    if (r == NULL) {
        return NULL;
    }
*/
